...
import { dirname, relative, resolve, extname, format } from 'path'
...
export function replaceTypescriptPaths({
    out: flagOut,
    project = 'tsconfig.json',
    src: flagSrc,
    verbose = false,
    ...
    const loadConfig = (file: string): ITSConfig => {
        const fileToParse = readFileSync(file, "utf-8")
